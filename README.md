...
- The result card draws the ascendancy passive tree the way poe2db and poedb do: the round class art with the nodes and their connections on top. Hovering or tapping a node opens a Path-of-Exile-style tooltip with its name and effect in the selected language. A node that grants a skill — in both games, whether it grants, triggers or supports it — shows it the way poe2db does: the skill icon on the line that names it and the skill's gem card beside the node card — tags, level, cost, cooldown, description, the stats of each part of the skill and the quality bonus. The cards sit side by side where the screen has room; a node with two skills (Chieftain's Tukohama, War's Herald) puts its own card on top and the two skills side by side under it when that fits better, and on a phone everything stacks. The skill name in the node line is highlighted only where the line spells it exactly; a declined form (Russian *Метки убийцы*) keeps just the icon. An effect the node grants that has a description of its own — Sands of Time, Jade, Combo, Tailwind, Volatility, the Flames of Chayula and the like, 41 in Path of Exile 2 and 2 in Path of Exile — gets a card of its own with that description, taken from the keyword popups of poe2db and poedb, and its word in the node line is underlined. Generic game terms the sites also link (Hit, Armour, Spell…) are left out, so a node never turns into a stack of glossary cards.
...
Each file holds the class list, the ascendancy and character names in 9 languages, and per tree the viewBox, node coordinates, connections and the name plus description of every node in 9 languages. System lines are left out of the node text: the internal stat ids the sites keep hidden (`max jade stacks [10]`) and the few bare untranslated identifiers they print as is (`generic_buff_aura`) — 35 lines in Path of Exile 2, 8 in Path of Exile.

- `data/poe2-skills/<lang>.js` — the 55 skills Path of Exile 2 ascendancy nodes grant, from the gem popups on poe2db.tw, and 41 effects with a description of their own, from its keyword popups (`k:1`, with `a` the words the node text uses for them).
- `data/poe1-skills/<lang>.js` — the 12 skills and support gems Path of Exile ascendancy nodes grant or trigger, from the gem popups on the poedb.tw skill pages, and 2 effects from its keyword popups.

One file per language, so only the current one is fetched, and only once a tree that needs it is shown. A node lists its skills and effects as `[line, key]` pairs (`s` in the node text), the line being the one that names them.

Where the source has no translation the page falls back to English. That is Thai throughout the Path of Exile 2 node text, and the Devotion to the King node in five languages. The same goes for the skill that node grants, Queen's Procession.
...
- `assets/<game>/skills/*.webp` — the icons of the granted skills, 55 for Path of Exile 2 and 12 for Path of Exile.
